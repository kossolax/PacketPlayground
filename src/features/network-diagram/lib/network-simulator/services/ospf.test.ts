...
describe('OSPFService - RFC 2328 Compliance', () => {
  ...
  describe('Router ID Generation', () => {
    it('should auto-generate router ID from highest interface IP', () => {
      ...
      // Highest IP in our setup is 192.168.1.1
      expect(ospfService.routerID.toString()).toBe('192.168.1.1');
    });
  ...
  describe('Network Statements', () => {
    ...
    it('should not enable OSPF on non-matching interfaces', () => {
      ...
      // eth1 has 192.168.1.1, should not match
  ...
  describe('Neighbor Management', () => {
    ...
    it('should discover neighbors from Hello messages', () => {
      const iface = router.getInterface('GigabitEthernet0/0');
      const config = ospfService.getInterfaceConfig(iface);
      expect(config).not.toBeNull();

      // Simulate receiving Hello message from neighbor
      ...
      ospfService.receivePacket(helloMessages[0]);

      // Should have discovered one neighbor
      const neighbors = ospfService.getAllNeighbors();
      expect(neighbors).toHaveLength(1);
      expect(neighbors[0].neighborID.equals(new IPAddress('2.2.2.2'))).toBe(true);
    });

    it('should transition neighbor to Init state on first Hello', () => {
      const iface = router.getInterface('GigabitEthernet0/0');

      // Build and process Hello message
      const helloBuilder = new OSPFHelloMessage.Builder();
      const helloMessages = helloBuilder
        .setPayload('hello')
        .setNetSource(new IPAddress('10.0.0.2'))
      ...
      const neighbors = ospfService.getAllNeighbors();
      // RFC 2328: Should transition from Down to Init when Hello received
      expect(neighbors[0].state).toBe(OSPFState.Init);
    });

    it('should transition to TwoWay when router sees itself in neighbor list', () => {
      const iface = router.getInterface('GigabitEthernet0/0');

      // First Hello without our router ID
      const hello1 = new OSPFHelloMessage.Builder()
        .setPayload('hello')
        .setNetSource(new IPAddress('10.0.0.2'))
        .setNetDestination(OSPF_ALL_ROUTERS)
        .setRouterID(new IPAddress('2.2.2.2'))
      ...
      ospfService.receivePacket(hello1[0]);

      let neighbors = ospfService.getAllNeighbors();
      expect(neighbors[0].state).toBe(OSPFState.Init);

      // Second Hello including our router ID in neighbor list
      const hello2 = new OSPFHelloMessage.Builder()
        .setPayload('hello')
        .setNetSource(new IPAddress('10.0.0.2'))
        .setNetDestination(OSPF_ALL_ROUTERS)
        .setRouterID(new IPAddress('2.2.2.2'))
        .setAreaID(new IPAddress('0.0.0.0'))
        .setNetworkMask(new IPAddress('255.255.255.0'))
        .setNeighbors([ospfService.routerID]) // Include our router ID
        .build();

      ospfService.receivePacket(hello2[0]);

      neighbors = ospfService.getAllNeighbors();
      // RFC 2328: Transition to TwoWay when we see ourselves in neighbor list
      expect(neighbors[0].state).toBe(OSPFState.TwoWay);
    });

    it('should extract DR and BDR from Hello messages', () => {
      ...
      const neighbors = ospfService.getAllNeighbors();
      expect(neighbors[0].designatedRouter.equals(dr)).toBe(true);
      expect(neighbors[0].backupDesignatedRouter.equals(bdr)).toBe(true);
    });

    it('should get neighbors by interface', () => {
      const iface = router.getInterface('GigabitEthernet0/0');

      // Add neighbor on eth0
      const helloBuilder = new OSPFHelloMessage.Builder();
      const helloMessages = helloBuilder
        .setPayload('hello')
        .setNetSource(new IPAddress('10.0.0.2'))
      ...
      const neighbors = ospfService.getNeighborsByInterface(iface);
      expect(neighbors).toHaveLength(1);
    });
    ...
    it('should ignore Hello messages from different areas', () => {
      ...
      const neighbors = ospfService.getAllNeighbors();
      // Should not add neighbor from different area
      expect(neighbors).toHaveLength(0);
    });
  });
  ...
  describe('OSPF State Machine', () => {
    ...
    it('should follow RFC 2328 state progression: Down -> Init -> TwoWay', () => {
      ospfService.Enable = true;
      ospfService.addNetwork(
        new IPAddress('10.0.0.0'),
        new IPAddress('0.0.0.255'),
      ...
      // Initial state: Down (no neighbors)
      expect(ospfService.getAllNeighbors()).toHaveLength(0);

      // Receive Hello -> Transition to Init
      const hello1 = new OSPFHelloMessage.Builder()
        .setPayload('hello')
        .setNetSource(new IPAddress('10.0.0.2'))
        .setNetDestination(OSPF_ALL_ROUTERS)
      ...
      // Receive Hello with our ID -> Transition to TwoWay
      const hello2 = new OSPFHelloMessage.Builder()
        .setPayload('hello')
        .setNetSource(new IPAddress('10.0.0.2'))
        .setNetDestination(OSPF_ALL_ROUTERS)
        ...
        .setAreaID(new IPAddress('0.0.0.0'))
        .setNetworkMask(new IPAddress('255.255.255.0'))
        .setNeighbors([ospfService.routerID])
        .build();
  ...
  describe('Enable/Disable OSPF', () => {
    it('should be disabled by default', () => {
      expect(ospfService.Enable).toBe(false);
    });

    it('should allow enabling OSPF', () => {
      ospfService.Enable = true;
      expect(ospfService.Enable).toBe(true);
    });
